 ...
 *  under the License.
 */
package eu.scape_project.tb.bptde;
...
/**
 * Book Page Tuples Detection Evaluation against ground truth. The evaluation
 * process first creates the detection result and ground truth lists. It then
 * counts each page tuple from the detection result that is in the ground truth
 * as correctly identified tuple (true positive). Those that are not in the
 * ground truth are counted as incorrectly identified tuples (false positives),
 * and finally, those that are in the ground truth but not in the detection
 * result are counted as missed tuples (false negatives). The precision is then
 * calculated as the number of true positives (i.e. the number of items
 * correctly labeled as duplicate page pairs) divided by the total number of
 * elements assumed to be duplicate page pairs (i.e. the sum of true positives
 * and false positives, which are items incorrectly labeled as being duplicate
 * page pairs ). Recall is then defined as the number of true positives divided
 * by the total number of elements of duplicate page pairs (i.e. the sum of true
 * positives and false negatives, which are items have not been labeled as being
 * duplicate page pairs but actually should have been). The ground truth
 * contains single page instances without duplicates and n-tuples (duplicates,
 * triples, quadruples, etc.). n-tuples with n>2 are expanded, the result is a
 * list of 2-tuples which is used to determine the number of missed duplicates
 * (false negatives).
 *
 * @author Sven Schlarb https://github.com/shsdev
 * @version 0.1
 */
public class BookPageTuplesDetectEval {
    ...
    private String detectionResult; // detection result
    ...
    /**
     * DetectionResult output content
     *
     * @return DetectionResult output content
     */
    ...
    /**
     * Getter for correctly identified duplicates (true positives). Number of
     * "true positives", which is the number of page pairs correctly identified
     * as being duplicates.
     *
    ...
    /**
     * Getter incorrectly identified duplicates (false positives). Number of
     * "false positives", which are page pairs that are incorrectly labeled as
     * being page pairs but in fact the are not.
     *
    ...
    /**
     * Getter for missed duplicates (false negatives). Number of "false
     * negatives", which are items that were not labeled as being page pairs but
     ...
     *
     * @param mb DetectionResult output
     * @param gt Ground truth
     */
    public BookPageTuplesDetectEval(String mb, String gt) {
        this.detectionResult = mb;
        this.groundTruth = gt;
        log = "";
    }

    /**
     * Evaluation process. The evaluation process first creates the detection
     * result and ground truth lists. It then counts each page tuple from the
     * detection result that is in the ground truth as correctly identified tuple
     ...
     * are in the ground truth but not in the detection result as missed tuples
     * (false negatives).
     *
    ...
    /**
     * Create a list from the detection result.
     *
     ...
     * @return list from the detection result
     */
    protected ArrayList<ArrayList<Integer>> getDetectionResultList(String result, String lineCond, int... positions) {
